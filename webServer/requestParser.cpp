...
Down:
    ...
    response_request(path,mime);
}

void Parser::response_request(const string& g_file, const string& mime) {
...
void Parser::connectRedis(){
    if(redis_->isRedisConnectWell_ == false){
        redis_->isRedisConnectWell_ = redis_->Connect();
        LOG_DEBUG << "connect to redis [" << redis_->isRedisConnectWell_<<"]";
